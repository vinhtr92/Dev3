...
class JSNUniformModelForm extends JModelItem
{
	...
	private function _save($dataForms, &$return, $post, $submissionsData, $dataContentEmail, $nameFileByIndentifier, $requiredField, $fileAttach, $recepientEmail)
	{
		...
		// Check if a form is payment gateway form, then check get all payment gateway settings
		if ((string) $dataForms->form_payment_type != '')
		{
			if (JPluginHelper::isEnabled('uniform', (string) $dataForms->form_payment_type))
			{
				JPluginHelper::importPlugin('uniform', (string) $dataForms->form_payment_type);
				$dispatcher = JEventDispatcher::getInstance();
				$plgPaymentGatewayParams = $dispatcher->trigger('getPaymentGatewayParams');
				if (count($plgPaymentGatewayParams))
				{
					$plgPaymentGatewayParams = $plgPaymentGatewayParams[0];
					if (is_object($plgPaymentGatewayParams))
					{
						// only send email when transaction done
						if (isset($plgPaymentGatewayParams->receive_confirmation_of_successful_transaction))
						{
							if ($plgPaymentGatewayParams->receive_confirmation_of_successful_transaction == '1')
							{
								$isSent = false;
							}
						}
